...
class Categoria(models.Model):
    ...
    valor_planejado = models.FloatField(default=0, blank=True)
    ...
    def total_gasto(self):
        ...
        valores = Valores.objects.filter(categoria__id = self.id).filter(tipo="S").filter(data__month=datetime.now().month).aggregate(Sum('valor'))
